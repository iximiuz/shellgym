...
tasks:
  discovered:
    ...
    hint: |
      echo "Ask uptime itself with uptime --help (or man uptime) and look for 'since'."
    solve: |
      uptime --help
  applied:
    needs: [discovered]
    timeout: 60
    check: |
      wait_exec "(^|/)uptime (-s|--since)\$"
    hint: |
      echo "It is an option with no value: uptime, then the option that prints the time the system has been up since."
    solve: |
      uptime -s
  looked_up:
    timeout: 60
...
1. The `uptime` command can print the exact date and time the machine was started.
   Find that option yourself, in `--help` or in the manual page, and
...
::task{name="applied"}
#active
Waiting for the time the machine was started...
#completed
