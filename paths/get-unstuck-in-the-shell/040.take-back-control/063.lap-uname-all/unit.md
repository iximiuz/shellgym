...
tasks:
  discovered:
    check: |
      wait_exec '(^|/)(uname --help|man uname)$'
    hint: |
      echo "Ask uname itself with uname --help (or man uname) and look for 'all'."
    solve: |
      uname --help
  applied:
    needs: [discovered]
    timeout: 60
    check: |
      wait_exec "(^|/)uname (-a|--all)\$"
    hint: |
      echo "It is an option with no value: uname, then the option that prints all information."
    solve: |
      uname -a
  looked_up:
    timeout: 60
...
1. The `uname` command prints the name of the operating system, and it can print
   everything it knows about the system on one line. Find that option
   yourself, in `--help` or in the manual page, and print all of it.
...
#active
Waiting for you to consult the documentation of `uname`...
#completed
...
::task{name="applied"}
#active
Waiting for `uname` to print everything it knows...
#completed
