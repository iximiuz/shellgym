...
tasks:
  discovered:
    check: |
      wait_exec '(^|/)(date --help|man date)$'
    hint: |
      echo "Ask date itself with date --help (or man date) and look for 'UTC'."
    solve: |
      date --help
  applied:
    needs: [discovered]
    timeout: 60
    check: |
      wait_exec "(^|/)date (-u|--utc|--universal)\$"
    hint: |
      echo "It is an option with no value: date, then the UTC option."
    solve: |
      date -u
  looked_up:
    timeout: 60
...
1. The `date` command can print the current time in UTC instead of the local time
   zone. Find that option yourself, in `--help` or in the manual page,
   and print the time in UTC.
...
#active
Waiting for you to consult the documentation of `date`...
#completed
...
::task{name="applied"}
#active
Waiting for the current time in UTC...
#completed
