...
tasks:
  ...
  arrived:
    ...
    hint: |
      echo "Put a backslash right before the space, or type cd projects/Vend and press Tab to let the shell do it."
    solve: |
      cd projects/Vendor\ Contracts
---

Quotes protect a whole word. A **backslash** protects a single
character: `Vendor\ Contracts` tells the shell that this one space is
part of the name.

You rarely need to type that by hand. Start in your home directory, type
`cd projects/Vend`, and press **Tab**. The shell completes the name and
...
::task{name="arrived"}
#active
Waiting for a backslash-escaped `cd` into `Vendor Contracts`...
#completed
