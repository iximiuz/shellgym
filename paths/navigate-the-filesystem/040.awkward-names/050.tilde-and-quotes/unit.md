...
tasks:
  ...
  arrived:
    ...
    hint: |
      echo "Start the path with the bare tilde and quote only the last part: ~/work/ followed by the quoted name. Or type ~/work/Proj and press Tab."
    solve: |
      cd ~/work/"Project Plans"
---

Combine two things you know. From `/tmp`, get into `~/work/Project
Plans` with a single command that starts with the tilde.

There is a trap. Quotes turn off the tilde: `"~/work/Project Plans"`
...
::task{name="arrived"}
#active
Waiting for a single `cd` from `~` into `Project Plans`...
#completed
The tilde expanded, the quotes held the space, and `cd` got one clean path.
::

::hint{title="No such file or directory: ~/work/Project Plans"}
