...
tasks:
  ...
  arrived:
    ...
    timeout: 60
    check: |
      wait_cwd "$SHELL_PID" "$GYM_USER_HOME/work/2026 \\(draft\\)" >/dev/null || exit 1
      LINE=$(wait_line --latest '^cd( +.*)?$') || exit 1
      ARG=$(printf '%s' "$LINE" | sed -E 's/^cd +//')
      case "$ARG" in
        *\"*|*\'*|*\\*) exit 0 ;;
        *) hint_exit "You got in, but the name was not protected. Go back to ~/work and quote or escape it." ;;
      esac
    hint: |
...
The `~/work` directory holds a directory named `2026 (draft)`. Start
in `~/work` and move into it. Quotes work, and so does Tab.

::task{name="at_work"}
#active
Waiting for your shell to be in `~/work`...
#completed
You are in `~/work`. Now enter the directory with the parentheses.
::
