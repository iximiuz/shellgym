...
tasks:
  ...
  arrived:
    ...
    timeout: 60
    check: |
      wait_cwd "$SHELL_PID" "$GYM_USER_HOME/projects/2026 \\(draft\\)" >/dev/null || exit 1
      LINE=$(wait_line --latest '^cd( +.*)?$') || exit 1
      ARG=$(printf '%s' "$LINE" | sed -E 's/^cd +//')
      case "$ARG" in
        *\"*|*\'*|*\\*) exit 0 ;;
        *) hint_exit "You got in, but the name was not protected. Go back to ~/projects and quote or escape it." ;;
      esac
    hint: |
      echo "Quote the whole name, or type cd 20 and press Tab to get it escaped."
    solve: |
      cd "2026 (draft)"
---

Spaces are the most common troublemaker in names, but far from the
only one. Parentheses, `&`, `;`, `*`, `?`, and a few other characters
all mean something to the shell, and a name containing them needs
the same protection.

The `~/projects` directory holds a directory named `2026 (draft)`. Start
in `~/projects` and move into it. Quotes work, and so does Tab.

::task{name="at_work"}
#active
Waiting for your shell to be in `~/projects`...
#completed
You are in `~/projects`. Now enter the directory with the parentheses.
::

::task{name="arrived"}
#active
Waiting for a protected `cd` into `2026 (draft)`...
#completed
The shell passed the parentheses through untouched.
::

::hint{title="Syntax error near unexpected token"}
That is what an unprotected `(` gets you. The shell tried to read the
parenthesis as part of its own syntax before `cd` ever ran. Quote the
name, or escape each awkward character with a backslash.
::
