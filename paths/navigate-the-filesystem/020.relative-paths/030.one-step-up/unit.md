...
tasks:
  at_start:
    check: |
      P=$(wait_cwd "$GYM_USER_HOME/projects/$START") || exit 1
      set_var SHELL_PID "$P"
    hint: |
      echo "Start in ~/projects/${START}. From anywhere, cd ~/projects/${START} gets you there."
    solve: |
      cd ~/projects/$START
  arrived:
    needs: [at_start]
    timeout: 60
    check: |
      wait_cwd "$SHELL_PID" "$(dirname "$GYM_USER_HOME/projects/$START")" >/dev/null || exit 1
...
Start in `~/projects/${START}`. Then move one level up, into the parent,
using the two-dot name.

::task{name="at_start"}
#active
Waiting for your shell to be in `~/projects/${START}`...
#completed
