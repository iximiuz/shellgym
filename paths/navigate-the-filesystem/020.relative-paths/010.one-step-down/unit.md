...
Start in your home directory. The `work` directory is right inside
...
Move into `work` by its name alone.

::task{name="at_home"}
#active
Waiting for your shell to be in your home directory...
#completed
You are home. Now step into `work`.
::

::task{name="arrived"}
#active
Waiting for a relative `cd` into `work`...
#completed
The shell found `work` inside the current directory.
::

::tip
Type `cd w` and press **Tab**. The shell completes the name for you,
because `work` is the only name here that starts with `w`.
::
