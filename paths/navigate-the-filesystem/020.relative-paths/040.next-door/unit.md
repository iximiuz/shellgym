...
tasks:
  at_start:
    check: |
      P=$(wait_cwd "$GYM_USER_HOME/projects/reports/final") || exit 1
      set_var SHELL_PID "$P"
    hint: |
      echo "Start in ~/projects/reports/final. From anywhere, cd ~/projects/reports/final gets you there."
    solve: |
...
Start in `~/projects/reports/final`. Then move to `drafts` with a
**single** relative path that goes up to the parent and down into the
sibling.

::task{name="at_start"}
#active
Waiting for your shell to be in `~/projects/reports/final`...
#completed
