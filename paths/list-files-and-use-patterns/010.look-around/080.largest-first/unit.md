...
tasks:
  sorted:
    ...
    check: |
      ...
      SL=' --sort=size '
      if [[ " $ARGV " =~ $S ]] || [[ " $ARGV " =~ $SL ]]; then exit 0; fi
      hint_exit "That listing is not sorted by size. Look for 'size' among the sort options in ls --help. The short form is an uppercase letter."
    hint: |
      ...
      echo "Look for 'sort by' and 'size' in ls --help. The short form is an uppercase letter, and it combines with -l and -h."
    solve: |
      ls -lhS ~/projects/data
---

Reading four sizes is quick, but a directory can hold hundreds of
files. It is faster to sort by size, largest first, and read the top
line.

List the contents of `~/projects/data` sorted by size. Readable sizes
help here too.

::task{name="sorted"}
#active
Waiting for a listing sorted by size...
#completed
The top line is the largest file in the directory.
::

::tip
Short options combine, so the long format and the readable sizes fit
in one argument. Many people type that combination so often that it
becomes automatic.
::
